...
public class BubbleSortCustomerData {
  ...
  /**
   * Bubble sort is a comparison-based algorithm in which each pair of adjacent
   * elements in an array is compared and the elements are swapped if they are
   * not in order. Implements the bubbleSwap method above to swap the elements.
   * The array is first bubble sorted into ID in ascending order, then by name
   * in descending order. The descending name bubble sort operation is
   * conditional on the customer ratings being equal, i.e. each rating group's
   * names will be sorted independently from the other groups, so the initial
   * rating bubble sort is unaffected.
   *
   */
  ...
  public static void main(String[] args) {
    ...
    // Calling BubbleSortRating method
